...
const validateReservation = (reservation) => {
  ...
  let invalidInputs = [];
  if (!car_id) {
    invalidInputs.push("pasirinkite automobilį");
  }
  if (!carTitle) {
    invalidInputs.push("pasirinkite automobilį");
  }
  if (!dateRented) {
    invalidInputs.push("pasirinkite nuomos datą");
  }
  if (!dateReturned) {
    invalidInputs.push("pasirinkite grąžinimo datą");
  }
...
export const getReservations = async (req, res) => {
  ...
  if (userCheck.isAdmin) {
    ...
    try {
      const reservations = await Reservation.find({ user_id }).sort({
        dateRented: -1,
      });
      res.status(200).json(reservations);
    } catch (err) {
...
export const createReservation = async (req, res) => {
  ...
  if (invalidInputs.length > 0) {
    return res
      .status(400)
      .json({ error: "Prašome užpildyti visus laukelius", invalidInputs });
  }
...
export const updateReservation = async (req, res) => {
  ...
  if (
    ...
    invalidInputs.push("pasirinkite statusą");
  }
  ...
  try {
    ...
    if (!reservation) {
      return res
        .status(404)
        .json({ error: "Paredaguoti rezervacijos nepavyko" });
    }
    res.status(200).json(reservation);
  } catch (err) {
    return res.status(500).json("Serverio klaida");
